...
import SideDrawer from "~/core/components/SideDrawer";
import { getNavbarListByRole, paths, TABS_ARRAY } from "~/core/utils/helper";
...
export default function DashboardLayout({ children }: MainlayoutProps) {
  ...
  const navSelection = (pathStr: string) => {
    const homeRoutes = [
      paths.homePage,
      paths.packageManagement,
      paths.invoices,
      paths.user,
      paths.roles,
    ];

    if (homeRoutes.includes(pathStr)) {
      return "primary.green";
    } else if (router.pathname === pathStr) {
      return "primary.green";
    } else {
      return "primary.typoBlack";
    }
  };
