...
const ResetPassword: NextPage = () => {
  ...
  const {
    ...
    validateResetPasswordLink(input)
  );
  ...
  } = useMutation((input: resetPasswordType) => resetPasswordLink(input));

  useEffect(() => {
    if (webEmail && webtoken) {
      validateResetPassword({ email: webEmail, token: webtoken });
    }
  ...
  useEffect(() => {
    if (resetPasswordSuccess) {
      handleToastMessage(labels.passwordResetSuccess);
      router.push(paths.loginPage);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const onSubmitHandler = () => {
    ...
    if (captcha) {
      resetPassword({ email: webEmail!, password, token: webtoken! });
    }
  };
